...
public class TaskProofListener {
    ...
    public TaskProofListener(ObjectMapper objectMapper, AiValidator aiValidator, TaskRepository taskRepository) {
    ...
    private void handleEvent(ProofSubmittedEvent event) throws ExecutionException, InterruptedException {
        ...
        Task task = taskRepository.findById(event.getChildId(), event.getTaskId()).get().toObject(Task.class);
        if (task != null) {
            ...
            logger.info("Task '{}' for child '{}' has been AI validated with result: {}", task.getDescription(), event.getChildId(), isValid);
        } else {
            logger.warn("Task with ID '{}' not found for child '{}'", event.getTaskId(), event.getChildId());
        }
    }
}

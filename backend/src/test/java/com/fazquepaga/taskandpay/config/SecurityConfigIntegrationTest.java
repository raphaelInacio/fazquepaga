...
class SecurityConfigIntegrationTest {
    ...
    // startup errors
    @MockitoBean private com.fazquepaga.taskandpay.security.RecaptchaService recaptchaService;
    @MockitoBean private com.fazquepaga.taskandpay.security.JwtService jwtService;
    @MockitoBean private com.fazquepaga.taskandpay.security.RefreshTokenService refreshTokenService;
    ...
    void publicEndpoints_shouldBeAccessible() throws Exception {
        mockMvc.perform(get("/api/v1/auth/login")) // POST required but access should be allowed
                // (400 or 405, not
                // 403/401)
                .andExpect(
                        result -> {
        ...
        mockMvc.perform(get("/index.html")).andExpect(status().isOk()); // If static resource exists
    }
                        ...
                        result -> {
                            ...
                            if (status == 401 || status == 403) {
                                throw new AssertionError(
                                        "Authenticated request blocked: " + status);
                            }
                        });
    }
}

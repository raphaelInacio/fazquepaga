...
class RateLimitIntegrationTest {
    ...
    @MockitoBean private AIQuotaService aiQuotaService;

    @MockitoBean private AiSuggestionService aiSuggestionService;

    @BeforeEach
    void setup() throws Exception {
        when(recaptchaService.verify(any(), any())).thenReturn(true);
        when(identityService.authenticateParent(any(), any()))
                .thenReturn(
                        User.builder()
                                .id("test-id")
                                .name("Test User")
    ...
    void shouldRateLimitGlobalRequests() throws Exception {
        if (rateLimitConfig.getGlobalLimit() != 10) {
            throw new IllegalStateException(
                    "Config not loaded correctly: " + rateLimitConfig.getGlobalLimit());
        }
                            ...
                            result -> {
                                if (result.getResponse().getStatus() == 429) {
                                    throw new AssertionError(
                                            "Premature 429 exceeded at request " + finalI);
                                }
                            });
        }
    ...
    void shouldRateLimitAIRequests_perUser() throws Exception {
        ...
        // AI limit is 3.
        for (int i = 0; i < 3; i++) {
            mockMvc.perform(get("/api/v1/ai/tasks/suggestions?age=10").with(user(mockUser)))
                    .andExpect(status().isOk());
        }

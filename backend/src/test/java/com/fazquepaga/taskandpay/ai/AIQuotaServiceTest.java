...
class AIQuotaServiceTest {
    ...
    @InjectMocks private AIQuotaService aiQuotaService;
    ...
    void shouldResetQuota_OnNewDay() throws ExecutionException, InterruptedException {
        ...
        AIQuota quota =
                AIQuota.builder()
                        .userId(USER_ID)
                        .usedToday(5)
                        .lastResetDate(LocalDate.now().minusDays(1).toString())
                        .dailyLimit(5)
                        .build();
        when(aiQuotaRepository.findByUserId(USER_ID)).thenReturn(quota);

        User freeUser =
                User.builder().id(USER_ID).subscriptionTier(User.SubscriptionTier.FREE).build();
        ...
        assertEquals(LocalDate.now().toString(), savedQuota.getLastResetDate());
    }
    ...
    void shouldCreateNewQuota_WhenNoneExists() throws ExecutionException, InterruptedException {
        ...
        assertEquals(LocalDate.now().toString(), createdQuota.getLastResetDate());
    }
}

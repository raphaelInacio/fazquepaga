...
### GitHub Pipeline
### GitHub Pipeline
- **Backend**: `CI/CD Pipeline` triggers on push to `main` (build, test, deploy).
- **Frontend**: `Frontend Deploy` triggers on push to `main` (build, deploy to Firebase).
- Status checks required: build success, tests pass, coverage >= 60%.
